...
import com.sk.op.api.common.dto.PageRequestDto;
import com.sk.op.api.common.dto.PageResultDto;
...
public class AdminController {
    ...
    public AdminController(RoleService roleService, MenuService menuService, RoleMenuRelationService roleMenuRelationService, AdminService adminService) {
    ...
    public ResponseDto<List<MenuTreeDto>> roleMenus(@Positive Long roleId) {
        List<RoleMenuRelation> relations = roleMenuRelationService.getAll(roleId);

        Map<Long, Menu> roleMenuMap = relations.stream().collect(Collectors.toMap(RoleMenuRelation::getMenuId, RoleMenuRelation::getMenu));

        List<Menu> menus = menuService.getAll(State.enable);
        Map<Long, MenuTreeDto> treeMap = menus.stream().map(menu -> new MenuTreeDto(menu, roleMenuMap.containsKey(menu.getId()))).collect(Collectors.toMap(MenuTreeDto::getId, tree -> tree));
    ...
    public ResponseDto<?> setRoleMenus(@RequestBody @Valid RoleSetMenusDto dto) {
        roleMenuRelationService.saveRoleMenus(dto);
        return ResponseDto.succeed();
    }
